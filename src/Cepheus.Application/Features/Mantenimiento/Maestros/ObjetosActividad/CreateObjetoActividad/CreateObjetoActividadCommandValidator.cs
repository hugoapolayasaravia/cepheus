...
{
    public class CreateObjetoActividadCommandValidator : AbstractValidator<CreateObjetoActividadCommand>
    {
        private readonly IUnitOfWork _uow;

        public CreateObjetoActividadCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;
            ...
            RuleFor(x => x.Code)
                ...
                .MustAsync(BeUniqueCode).WithMessage("Ya existe un objeto de actividad con ese código.");

            RuleFor(x => x.Name)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage("El nombre del objeto de actividad es obligatorio.")
                .MaximumLength(50).WithMessage("El nombre no puede exceder los 50 caracteres.")
                .MustAsync(BeUniqueName).WithMessage("Ya existe un objeto de actividad con ese nombre.");
        }
        ...
        private async Task<bool> BeUniqueCode(string code, CancellationToken cancellationToken)
        {
            ...
            return !await _uow.Mantenimiento.Maestros.ObjetosActividad.Query()
