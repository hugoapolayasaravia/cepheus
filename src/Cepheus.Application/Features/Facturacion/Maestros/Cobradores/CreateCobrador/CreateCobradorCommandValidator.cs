...
{
    public class CreateCobradorCommandValidator : AbstractValidator<CreateCobradorCommand>
    {
        private readonly IUnitOfWork _uow;

        public CreateCobradorCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;
        ...
        private async Task<bool> UserNotLinked(CreateCobradorCommand command, int? userId, CancellationToken ct)
            => !await _uow.Facturacion.Maestros.Cobradores.Query().AnyAsync(x => x.UserId == userId!.Value, ct);
    }
}
