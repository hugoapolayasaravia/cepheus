...
{
    public class UpdateCobradorCommandValidator : AbstractValidator<UpdateCobradorCommand>
    {
        private readonly IUnitOfWork _uow;

        public UpdateCobradorCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;
        ...
        private async Task<bool> UserNotLinked(UpdateCobradorCommand command, int? userId, CancellationToken ct)
            => !await _uow.Facturacion.Maestros.Cobradores.Query().AnyAsync(x => x.UserId == userId!.Value && x.Code != command.Code, ct);
    }
}
