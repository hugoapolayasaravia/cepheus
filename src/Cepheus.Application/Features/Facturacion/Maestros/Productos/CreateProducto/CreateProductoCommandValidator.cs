...
namespace Cepheus.Application.Features.Facturacion.Maestros.Productos.CreateProducto
{
    public class CreateProductoCommandValidator : AbstractValidator<CreateProductoCommand>
    {
        ...
        public CreateProductoCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;
            ...
            RuleFor(x => x.TipoProductoCode)
                ...
                .MustAsync(TipoProductoExists).WithMessage("El tipo de producto indicado no existe.");
            ...
            RuleFor(x => x)
                ...
                .OverridePropertyName(nameof(CreateProductoCommand.TransportCode));
        ...
        private void AddAttributeRule(Expression<Func<CreateProductoCommand, string?>> selector, TipoAtributoConcreto type, string label)
        {
        ...
        private async Task<bool> TransportProductExists(CreateProductoCommand command, CancellationToken ct)
        {
