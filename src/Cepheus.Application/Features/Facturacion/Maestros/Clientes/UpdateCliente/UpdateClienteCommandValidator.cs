...
namespace Cepheus.Application.Features.Facturacion.Maestros.Clientes.UpdateCliente
{
    public class UpdateClienteCommandValidator : AbstractValidator<UpdateClienteCommand>
    {
        ...
        public UpdateClienteCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;
            ...
            RuleFor(x => x.DocumentNumber)
                ...
                .MustAsync(BeUniqueDocumentNumber).WithMessage("Ya existe otro cliente con ese número de documento.");

            RuleFor(x => x.Name)
                .MaximumLength(120).WithMessage("El nombre no puede exceder los 120 caracteres.")
                .MustAsync(BeUniqueName).WithMessage("Ya existe otro cliente con ese nombre.")
            ...
            RuleFor(x => x)
                .Must(x => string.IsNullOrWhiteSpace(x.ParentClientCode) ||
                           !string.Equals(x.ParentClientCode.Trim(), x.Code.Trim(), StringComparison.OrdinalIgnoreCase))
                .WithMessage("Un cliente no puede ser su propio cliente matriz.")
                .OverridePropertyName(nameof(UpdateClienteCommand.ParentClientCode));
        ...
        private async Task<bool> BeUniqueDocumentNumber(UpdateClienteCommand command, string documentNumber, CancellationToken ct)
            => !await _uow.Facturacion.Maestros.Clientes.Query()
                .AnyAsync(c => c.Code != command.Code && c.DocumentNumber == documentNumber.Trim(), ct);

        private async Task<bool> BeUniqueName(UpdateClienteCommand command, string? name, CancellationToken ct)
            => !await _uow.Facturacion.Maestros.Clientes.Query()
                .AnyAsync(c => c.Code != command.Code && c.Name != null && c.Name.ToLower() == name!.Trim().ToLower(), ct);
