...
{
    public class UpdateTrabajadorFiscalCommandValidator
        : AbstractValidator<UpdateTrabajadorFiscalCommand>
    {
        private readonly IUnitOfWork _uow;

        public UpdateTrabajadorFiscalCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;

            RuleFor(x => x.Id)
                .GreaterThan(0)
                .WithMessage("El Id de la información fiscal es obligatorio.");
        ...
        private async Task<bool> TrabajadorFiscalDoesNotExist(
            UpdateTrabajadorFiscalCommand command,
            string trabajadorCode,
            CancellationToken cancellationToken)
        {
