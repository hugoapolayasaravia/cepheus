...
{
    public class UpdateTrabajadorVacacionCommandValidator
        : AbstractValidator<UpdateTrabajadorVacacionCommand>
    {
        private readonly IUnitOfWork _uow;

        public UpdateTrabajadorVacacionCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;

            RuleFor(x => x.Id)
                .GreaterThan(0)
                .WithMessage("El Id de la vacación es obligatorio.");
