...
{
    public class UpdateTrabajadorCuentaBancariaCommandValidator
        : AbstractValidator<UpdateTrabajadorCuentaBancariaCommand>
    {
        private readonly IUnitOfWork _uow;

        public UpdateTrabajadorCuentaBancariaCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;

            RuleFor(x => x.Id)
                .GreaterThan(0)
                .WithMessage("El id de la cuenta bancaria es obligatorio.");
            ...
            RuleFor(x => x)
                ...
                .When(x => x.Principal)
                .WithMessage("El trabajador ya tiene otra cuenta bancaria principal.");
        }
        ...
        private async Task<bool> NoExisteOtraCuentaPrincipal(
            UpdateTrabajadorCuentaBancariaCommand command,
            CancellationToken cancellationToken)
        {
