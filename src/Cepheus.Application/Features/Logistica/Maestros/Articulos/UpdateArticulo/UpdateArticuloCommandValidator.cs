...
namespace Cepheus.Application.Features.Logistica.Maestros.Articulos.UpdateArticulo
{
    public class UpdateArticuloCommandValidator : AbstractValidator<UpdateArticuloCommand>
    {
        ...
        public UpdateArticuloCommandValidator(IUnitOfWork uow)
        {
            _uow = uow;

            RuleFor(x => x.Code)
                .NotEmpty().WithMessage("El código del artículo es obligatorio.")
                .Length(7).WithMessage("El código debe tener 7 caracteres.");
            ...
            RuleFor(x => x)
                ...
                .OverridePropertyName(nameof(UpdateArticuloCommand.MaxStock));
